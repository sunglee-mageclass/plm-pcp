...
- **RPCs SECURITY DEFINER**: `search_path` fixo, checagem de tenant (`get_user_tenant_id()`/`is_super_admin()`/`is_tenant_admin()`), EXECUTE do `_core` revogado. ⚠️ **`REVOKE … FROM PUBLIC` NÃO basta** — o default ACL do Supabase concede EXECUTE a `anon`/`authenticated` em toda função nova; confirme com `has_function_privilege('authenticated','public._x_core(...)','execute')` = **false** e exija `REVOKE EXECUTE … FROM anon, authenticated` (senão dá pra chamar o `_core` direto e furar o gate de módulo do wrapper — regressão real pega no CQ Pós). Padrão wrapper + `_core`: wrapper checa `user_can_view(_pagina)` (dashboards) e `tenant_module_enabled(_module)` (módulos desligáveis).
