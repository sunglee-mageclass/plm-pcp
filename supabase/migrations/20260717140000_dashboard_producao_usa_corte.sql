-- Dashboard > Produção: os cards "Modelos cortados / Grade total cortada" só fazem
-- sentido quando a loja usa o SERVIÇO "Corte" (terceirizado). Se é tudo PL (corte
-- incluso no pacote), os números são redundantes com produção. A RPC passa a devolver
-- a flag `usaCorte` (existe bloco de serviço da categoria "Corte" ativo) e o front
-- esconde os dois cards quando false.
