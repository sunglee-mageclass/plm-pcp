-- Kanban: nova condição `grade_todas_variantes` — a grade está preenchida para TODAS as
-- variantes selecionadas do Tecido 1 (não só "alguma grade > 0"). N variantes do Tecido 1
-- (modelo_tecido_variantes com variante_tecido_id) ↔ N linhas de modelo_grades com
-- grade_total > 0 (variante_numero = posição 1..N).
