-- OTB: cada semana do card ganha um campo TEXTO + DATA (além da qtd). Colunas em
-- colecao_semanas + otb_salvar_colecao passa a gravar texto/data (payload weeksMeta / sub.meta).
alter table public.colecao_semanas add column if not exists texto text;
alter table public.colecao_semanas add column if not exists data date;
