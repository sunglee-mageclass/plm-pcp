...
-- A 3-arg tinha PUBLIC/anon revogados (20260623290000_revoke_anon_wrappers_escrita.sql); o
-- DROP+CREATE acima reseta a ACL pro default (PUBLIC executável) — restaura a blindagem
-- (não explorável, o corpo checa auth.uid(), mas preserva a defesa em profundidade).
REVOKE EXECUTE ON FUNCTION public.salvar_terceirizados(uuid, jsonb, text, jsonb) FROM PUBLIC, anon;
