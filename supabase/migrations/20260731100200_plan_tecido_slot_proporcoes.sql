-- 20260731100200_plan_tecido_slot_proporcoes.sql — Plan. Tecido: proporção de grade editável POR SLOT.
-- Antes a proporção era só leitura do modelo (não gravava, revertia ao digitar). Agora o slot tem
-- proporcoes própria: o save persiste e a árvore lê coalesce(sl.proporcoes, m.proporcoes).
begin;

alter table public.plan_tecido_slots add column if not exists proporcoes jsonb;
