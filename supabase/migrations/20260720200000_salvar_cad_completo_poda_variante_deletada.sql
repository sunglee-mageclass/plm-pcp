-- Ao DELETAR uma variante (cor) no Desenvolvimento, ela sobrevivia como FANTASMA:
--  - cad_grades: a reidrata da grade real (CQ confirmado) re-inseria a variante omitida;
--  - cq_variantes / cq_pos_variantes / direcionamento: nunca eram limpos.
-- Resultado: a variante deletada continuava aparecendo no CQ e no split do Direcionamento.
-- Correção: depois de reinserir os tecidos, PODA todas as variantes que não estão mais no
-- tecido-1 (a fonte das cores/variante_numero) de cad_grades/cq/cq_pos/direcionamento.
BEGIN;
...
BEGIN
  ...
  -- Limpa o que será re-inserido. modelo_grades/cad_grades SÓ quando o payload traz grade
  -- (v_has_grades) — senão a grade é PRESERVADA (não destrói por payload vazio; bug 1+4).
  ...
  IF v_has_grades THEN
    ...
    -- Reidrata as grades REAIS preservadas (CQ confirmado) de variantes OMITIDAS do _grades
    -- (ex.: planejada zerada). A poda no fim remove as que foram de fato DELETADAS.
    IF v_cq_confirmado THEN
  ...
  -- PODA: variante_numero válido = ordem das variantes do TECIDO 1 (fonte das cores). Remove
  -- de cad_grades/cq_variantes/cq_pos_variantes/direcionamento as variantes DELETADAS (que não
  -- estão mais no tecido-1) — senão a variante deletada vira fantasma no CQ e no Direcionamento.
  SELECT COALESCE(array_agg(DISTINCT ctv.ordem), '{}')
   ...
   WHERE ct.cad_id = v_cad_id AND ct.tipo = 'tecido' AND ct.numero = 1 AND ctv.ordem IS NOT NULL;

  DELETE FROM public.cad_grades
   WHERE cad_id = v_cad_id AND variante_numero <> ALL(v_valid_nums);
  ...
  DELETE FROM public.cq_variantes
   WHERE variante_numero <> ALL(v_valid_nums)
     AND controle_qualidade_id IN (SELECT id FROM public.controle_qualidade WHERE cad_id = v_cad_id);
  ...
  -- Sincroniza consumo e %loss do CAD de volta para o Desenvolvimento (BOM).
