-- Etapas PL S4, Task 2: salvar_terceirizados passa a persistir nf_saida/nf_entrada
-- (colunas jsonb adicionadas na Task 1) a partir do jsonb de cada bloco.
-- CREATE OR REPLACE sobre a definição VIVA da função (dump em /tmp/st_before.sql) —
-- a ÚNICA mudança é a adição das 2 colunas ao UPDATE SET (após pt_aprovacao) e ao
-- INSERT (lista de colunas + VALUES), mesmo padrão do trio pt_data_saida/pt_data_entrada/
-- pt_aprovacao feito na migration 20260821130000.
