-- Renomeia 2 rótulos LEGADOS do leadtime (set/2026): 'CAD → Corte' (processo de corte que a
-- Ave Rara não usa mais — na verdade é o marco 'Enviar para PCP' da Explosão) → 'CAD → Enviar
-- p/ PCP'; e 'Produção (Serviços)' → 'Tempo em produção' (é a fase inteira, não o total). SÓ
-- troca de LABEL no _dashboard_leadtime_core (skeleton de etapas) — cálculo/keys/ACL inalterados.
-- Diff-validar: só as 2 strings de label mudam. O front (dashboard.tsx) já casa esses rótulos.
...
BEGIN
  ...
  macro AS (
    ...
    UNION ALL
    SELECT 'cad_corte', 'macro', 'CAD → Enviar p/ PCP',
