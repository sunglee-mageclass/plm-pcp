-- Consumo por OC v3: por item de OC, lista TODOS os modelos que consomem (vínculo
-- cortado ou não + baixa FIFO sem vínculo), cada um com os números de BOM mesmo
-- após o corte (grade por variante/geral, consumo, multiplicador) + baixado real.
-- metragem_m = consumo × mult × grade_variante (SEM piloto). O piloto (+1) é somado
-- UMA vez por modelo no front. Cliente agrupa os itens por artigo (tecido) e junta
-- as variantes de cada modelo num card só.
            ...
            SELECT jsonb_build_object(
              ...
              'modelos', (
                SELECT COALESCE(jsonb_agg(m_row), '[]'::jsonb) FROM (
                  -- Vínculo explícito (cortado ou não).
                  SELECT jsonb_build_object(
                  ...
                  -- Baixa FIFO sem vínculo (mantém infos via BOM da variante).
                  SELECT jsonb_build_object(
                  ...
                  LEFT JOIN LATERAL (
                    ...
                    WHERE mt2.modelo_id = bxa.modelo_id AND mtv2.variante_tecido_id = it.variante_tecido_id
                    LIMIT 1
                  ) bom ON true
