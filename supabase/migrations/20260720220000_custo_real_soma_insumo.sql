-- Custo REAL (a partir do CAD) passa a somar INSUMOS (etiquetas), igual ao previsto (BOM).
-- Espelha precoEtiquetaCor do front: MAX(preco da variante na cor) senão etiquetas.preco; sem loss (=aviamento).
...
-- CREATE OR REPLACE preserva a ACL; re-revoga por garantia (invariante #9), assinatura correta.
