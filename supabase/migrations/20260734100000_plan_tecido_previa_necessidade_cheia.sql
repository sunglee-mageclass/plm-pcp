-- 20260734100000_plan_tecido_previa_necessidade_cheia.sql — Plan. Tecido: pedido = necessidade CHEIA.
-- Decisão do dono: cards marcados p/ COMPRAR (usar_estoque=false) pedem a necessidade INTEIRA, ignorando
-- o estoque (o pano que há pode já estar comprometido). O estoque_m segue no retorno como INFORMATIVO.
-- (usar_estoque=true continua fora da necessidade = usa o que tem, não pede.)
begin;
...
begin
  ...
  base as (
    select n.artigo_id, n.variante_tecido_id, n.nec_m,
           ...
           round(n.nec_m::numeric, 4) as deficit_m,   -- PEDIDO = necessidade cheia (estoque é só informativo)
