-- Faxina do banco: remove os resíduos da tela Acabamento (aposentada → serviço "pós-costura",
-- aba Pós em Serviços). A tabela public.producao_acabamento tem 0 linhas (nunca houve dado),
-- ninguém depende dela (nenhuma FK aponta pra ela) e o único writer (RPC salvar_acabamento,
-- chamada só pela rota já deletada) some junto.
--
-- 5 funções referenciavam producao_acabamento; 4 são atualizadas aqui e 1 (fn_audit) fica como
-- está de propósito: nela é só um literal de texto ('producao_acabamento' -> 'Acabamento') que
-- rotula linhas HISTÓRICAS do audit_log — não referencia a tabela, então dropar a tabela não a
-- quebra, e manter preserva o rótulo de eventuais registros antigos.
--
-- Como a tabela está vazia, remover os ramos que liam dela é BEHAVIOR-PRESERVING (contribuíam 0
-- linhas em todo agregado). Também dropo tenant_config.oficina_posicao: seu único propósito era o
-- toggle "oficina fica sob Acabamento vs Serviços" — retirado; hoje NENHUMA função do banco nem
-- código do front lê essa coluna (todos os 4 tenants = 'terceirizados').

-- 1) modelo_etapas_afetadas: tira a chave 'acabamento' do jsonb.
...
-- 2) marcar_revisao_por_mudanca: tira o ramo que marcava revisão em 'acabamento'.
...
-- 3) _dashboard_producao_core: tira o WHEN 'Acabamento' da timeline e os 2 ramos UNION ALL que
--    liam de producao_acabamento (SLA por terceirizado e KPI de prazo). Sem efeito no resultado
--    (tabela vazia); "Acabamento" já caía sob "Serviço" na UI da timeline.
...
-- 4) Dropa o writer morto (só a rota deletada chamava).
DROP FUNCTION IF EXISTS public.salvar_acabamento(uuid, jsonb);

-- 5) Dropa a tabela vazia. Leva junto seus 3 triggers (audit/auto_status/set_tenant_id na tabela)
--    e 7 policies RLS; nenhuma FK aponta pra ela. As funções de trigger são compartilhadas e ficam.
DROP TABLE IF EXISTS public.producao_acabamento;

-- 6) Dropa a coluna sem leitor. Era o toggle oficina↔acabamento, retirado.
ALTER TABLE public.tenant_config DROP COLUMN IF EXISTS oficina_posicao;
