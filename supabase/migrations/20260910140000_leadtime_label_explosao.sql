-- Corrige o rótulo da etapa 'cad_corte' no leadtime: era 'CAD → Enviar p/ PCP', mas a tela CAD
-- não existe mais e o span mede o TEMPO DENTRO DA EXPLOSÃO (do 'Enviar para CAD'=entrar na
-- Explosão até 'Enviar para PCP'=sair). Nome correto no vocabulário do sistema: 'Explosão'.
-- SÓ troca de LABEL no _dashboard_leadtime_core; cálculo/keys/ACL inalterados. Diff-validar.
