-- Consumo por OC: ignorar consumo de ENTRETELA (mostrar só tecido e forro).
-- O tipo vem do uso do tecido pelo modelo (modelo_tecidos.tipo / link.tipo).
            ...
            SELECT jsonb_build_object(
              ...
              'modelos', (
                SELECT COALESCE(jsonb_agg(m_row), '[]'::jsonb) FROM (
                  -- Vínculo explícito (cortado ou não). Ignora ENTRETELA.
                  SELECT jsonb_build_object(
                  ...
                  -- Baixa FIFO sem vínculo (mantém infos via BOM da variante). Ignora ENTRETELA.
                  SELECT jsonb_build_object(
