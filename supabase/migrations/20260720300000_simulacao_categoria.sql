-- Feature A1 (categoria no Simulador de OC, modo Orçamento): cada modelo/slot do simulador
-- passa a ter uma categoria (chip nos reais; vazios gerados por categoria a partir do plano
-- colecao_semana_categorias). Persiste otb_simulacao_modelos.categoria_id.
BEGIN;
  ...
  for v_un in select value from jsonb_array_elements(coalesce(_arvore,'[]'::jsonb)) loop
    ...
    for v_ln in select value from jsonb_array_elements(coalesce(v_un->'linhas','[]'::jsonb)) loop
      ...
      for v_md in select value from jsonb_array_elements(coalesce(v_ln->'modelos','[]'::jsonb)) loop
        insert into public.otb_simulacao_modelos (linha_ref_id, modelo_id, slot_index, consumo, prof_por_cor, categoria_id)
