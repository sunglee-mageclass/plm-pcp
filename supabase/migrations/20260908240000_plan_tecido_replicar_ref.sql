-- Plan.Tecido — REPLICAR mantém a MESMA REF (regra do dono: réplica não muda REF, só versiona).
-- Antes, o INSERT em `modelos` do _replicar_cards_plan_tecido_core OMITIA `ref`/`ref_auto` → a
-- réplica nascia com REF vazia e o trigger fn_modelo_ref_auto gerava uma REF NOVA ao chegar ao
-- Desenvolvimento. Paridade com replicar_produtos_acabados/importados (que já copiam o.ref).
--
-- Correção CIRÚRGICA: reproduz o corpo VIVO da v3 (20260903140000) byte-a-byte, mudando SÓ o
-- INSERT em `modelos` — acrescenta as colunas `ref` e `ref_auto` copiando `o.ref`/`o.ref_auto` do
-- modelo-fonte. Como a réplica nasce com `ordem_criacao_enviada = false`, o trigger
-- fn_modelo_ref_auto (guard: RETURN NEW cedo quando não enviado) NÃO sobrescreve a REF copiada;
-- e a invariante #11 garante que REF não-vazia nunca é re-sincronizada. Versionamento
-- (modelo_base_id/versao) intocado. Diff-validar com pg_get_functiondef antes/depois: a ÚNICA
-- mudança deve ser ref/ref_auto no INSERT. Sem backfill das réplicas antigas (decisão do dono).
