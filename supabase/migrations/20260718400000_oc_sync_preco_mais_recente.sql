-- Fase A (correção): o cadastro (variantes_tecido.preco) deve refletir o preço da OC MAIS RECENTE
-- por variante — NÃO o preço da OC que se está salvando. Assim, editar uma OC ANTIGA não muda o
-- cadastro se existe uma OC mais recente com aquela variante (o "atual" vem da compra mais recente).
-- Recomputa só as variantes tocadas no save, pegando a OC mais recente por data_pedido/created_at.
