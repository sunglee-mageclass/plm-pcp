-- Seleção de OC no corte: reservado de outros modelos recalculado
-- (consumo×(1+loss)×grade×mult), igual ao detalhe_estoque_variante, em vez de
-- ler modelo_tecido_oc_links.quantidade_m (que às vezes fica 0).
  ...
  FROM (
    SELECT
      ...
      CASE WHEN it.estoque_zerado THEN 0 ELSE (
        ...
        - COALESCE((
            -- reservado de OUTROS modelos: recalculado consumo×(1+loss)×grade×mult
            -- (mesma conta do detalhe_estoque_variante), não a coluna quantidade_m.
