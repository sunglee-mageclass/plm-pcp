-- Rolos não geram parcelas no Financeiro (não são compra).
...
DECLARE
  ...
  i integer;
BEGIN
  -- Rolos não são compra: não geram parcelas no Financeiro.
...
-- recalcular_parcelas também ignora rolos (defesa em profundidade).
...
BEGIN
  ...
  END IF;
  -- Rolos não são compra: nunca geram parcelas.
