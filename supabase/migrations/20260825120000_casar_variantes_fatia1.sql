-- Fatia 1 — "casar variantes de 2 tecidos": persistir o VÍNCULO entre a variante
-- de um bloco de tecido complementar e a variante X do Tecido 1.
--
-- Aditivo. Coluna NOVA nullable + gravar nas funções que salvam o BOM do
-- Desenvolvimento (_salvar_modelo_bom_core) e que espelham no CAD
-- (_enviar_modelo_para_cad_core). NÃO toca estoque/reserva/abate/consumo.
--
-- Padrão jsonb (confirmado no banco vivo): cada bloco de tecido `t` tem
--   t->'variantes'       (array de ids, na ordem)
--   t->'multiplicadores' (array paralelo, index v_idx-1)
-- A coluna nova segue o MESMO padrão paralelo:
--   t->'complementas'    (array paralelo, index v_idx-1 → uuid da variante do Tecido 1)
--
-- As outras 4 funções que inserem em modelo_tecido_variantes
-- (_plan_tecido_gravar_bom_core, _plan_tecido_aplicar_ao_modelo_core,
--  _aplicar_sim_no_modelo_core, _modelo_bom_snapshot) NÃO recebem `complementas`
-- nesta fatia → ficam intocadas (a coluna fica NULL nesses caminhos, o que é
-- correto — elas não casam variantes). Documentado no report da Task 1.

BEGIN;

-- Step 1: coluna nova nas duas tabelas (idempotente).
ALTER TABLE public.modelo_tecido_variantes
  ADD COLUMN IF NOT EXISTS complementa_variante_id uuid
  REFERENCES public.variantes_tecido(id) ON DELETE SET NULL;

ALTER TABLE public.cad_tecido_variantes
  ADD COLUMN IF NOT EXISTS complementa_variante_id uuid
  REFERENCES public.variantes_tecido(id) ON DELETE SET NULL;

-- Step 2: _salvar_modelo_bom_core — grava complementa_variante_id no INSERT das
-- variantes (SÓ o delta; corpo idêntico ao vivo).
...
-- Preserva a ACL do padrão _core (invariante #9): EXECUTE revogado dos TRÊS.
REVOKE EXECUTE ON FUNCTION public._salvar_modelo_bom_core(uuid, jsonb, jsonb, jsonb, integer) FROM PUBLIC, anon, authenticated;

-- Step 3: _enviar_modelo_para_cad_core — espelha complementa_variante_id no CAD
-- (SÓ o delta no INSERT…SELECT das variantes; corpo idêntico ao vivo).
  ...
  LOOP
    ...
    INSERT INTO public.cad_tecido_variantes
      (cad_tecido_id, variante_tecido_id, ordem, multiplicador,
       quantidade_folhas, metragem_planejada, metragem_enviada, complementa_variante_id)
    SELECT v_new_tid, mtv.variante_tecido_id, mtv.ordem,
           COALESCE(mtv.multiplicador, 1), 0, 0, 0, mtv.complementa_variante_id
