-- Kanban: a condição servico_aprovado passa a checar a aprovação da MÃO DE OBRA
-- por modelo (modelos.custo_terceirizados_aprovado), no lugar da aprovação por-bloco
-- (producao_terceirizados.aprovado, agora órfã). Mantém a mesma key p/ não quebrar
-- os requisitos já configurados (nem o teste anti-drift catálogo↔RPC).
