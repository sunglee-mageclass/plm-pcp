-- FIX (set/2026): "Modelos por etapa do kanban" (dashboard_producao) empilhava ~todos os modelos
-- na 1ª coluna. Causa: colunas de status_kanban salvas como STRING (ex.: "PCP", "Ficha de Explosão")
-- viravam key CRUA ("PCP"), mas modelos.status_desenvolvimento guarda o SLUG ("pcp",
-- "ficha_de_explosao") → nunca casavam → caíam no fallback (firstcol). Correção: derivar a key da
-- coluna-string via public._kanban_resolve_key() (mesma resolução do board / _kanban_status_rows /
-- resolveStatusKey do front), que slugifica e mapeia labels conhecidos. Só o bloco `cols` muda; o
-- resto do corpo é reproduzido byte-a-byte. Diff-validar. Reafirmar REVOKE (invariante #9).
-- Afeta a aba Produção E a nova aba Desenvolvimento (ambas leem kanbanDev).
