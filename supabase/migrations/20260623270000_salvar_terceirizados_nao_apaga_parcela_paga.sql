-- salvar_terceirizados, ao remover um bloco de serviço do CAD, fazia DELETE em
-- producao_terceirizados; a FK parcelas_servico ON DELETE CASCADE apagava junto
-- as parcelas, INCLUSIVE status='pago' -> sumia histórico financeiro sem trilha.
-- Correção: antes do DELETE, bloqueia (RAISE) se algum bloco a remover tem parcela
-- paga. Gerado por inserção do guard sobre o corpo vivo (sem retipar a função).
...
BEGIN
  ...
  -- Não apagar bloco de serviço com parcela PAGA: a FK parcelas_servico ON DELETE
  -- CASCADE levaria junto o histórico financeiro. Bloqueia com mensagem clara.
  IF EXISTS (
