-- Etiquetas material completo — FASE 1d: cor por etiqueta no CAD.
-- cad_etiquetas ganha cor_id (a cor escolhida no BOM/CAD). O tamanho NAO se guarda:
-- explode pela grade na exibicao. Recria _salvar_cad_completo_core (def viva) so com
-- cor_id no INSERT das etiquetas.

BEGIN;

ALTER TABLE public.cad_etiquetas ADD COLUMN IF NOT EXISTS cor_id uuid REFERENCES public.cores(id);
  ...
  IF jsonb_typeof(_etiquetas) = 'array' THEN
    FOR e IN SELECT value FROM jsonb_array_elements(COALESCE(_etiquetas, '[]'::jsonb)) LOOP
      INSERT INTO public.cad_etiquetas
        (cad_id, etiqueta_id, cor_id, consumo, quantidade_planejada, quantidade_enviar)
      VALUES
        (v_cad_id,
         (e->>'etiqueta_id')::uuid,
