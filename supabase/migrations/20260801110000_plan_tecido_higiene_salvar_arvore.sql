-- Plan. Tecido — higiene de migration: re-aplica _salvar/_arvore CORRETOS por cima dos arquivos
-- landmine (20260731100200 etc., timestamp posterior) que reverteriam num db push limpo. As
-- versões abaixo são as VIVAS/corretas (categoria_tecido_id + subcolecao_categorias + slot_oc
-- re-link + cor_id/cor_apelido_id + proporcoes + label cor-apelido). Idempotente.
