-- Explosão: persiste o déficit de estoque do último envio ao corte.
--
-- Antes, o array `deficit[]` que `_baixar_estoque_tecido_corte_core` já calculava só
-- aparecia no toast do Sheet — a lista de Explosão não tinha como mostrar "Faltou
-- estoque" depois que o usuário fechava o painel. Aditivo: coluna nullable +
-- mudança MÍNIMA no core (grava o mesmo array que a função já retorna; regrava/limpa
-- a cada envio, já que a função sempre recalcula o déficit do zero a cada chamada).
-- Contrato da função (assinatura, retorno, ACL) intocado — dump/diff-validado.
ALTER TABLE public.cad ADD COLUMN IF NOT EXISTS deficit_corte jsonb;
...
BEGIN
  ...
  -- Persiste o déficit deste envio (limpa quando não há déficit) — a lista de Explosão
  -- lê esta coluna p/ mostrar o chip "Faltou estoque" mesmo depois do Sheet fechar.
  -- Regravado do zero a cada chamada (idempotente com o recálculo acima).
  UPDATE public.cad
     SET deficit_corte = NULLIF(v_deficit, '[]'::jsonb)
   WHERE id = _cad_id;
...
-- ACL intocada — reafirma o REVOKE dos três (invariante #9), defesa em profundidade.
