-- Dashboard > Produção: "SLA por serviço" só mostrava fornecedores com empresa_id
-- (JOIN empresas + WHERE empresa_id IS NOT NULL) → serviço via representante/colaborador
-- (ou sem fornecedor) sumia. Agora o fornecedor vem de empresa/representante/colaborador
-- (COALESCE, LEFT JOIN), então todos os serviços entregues aparecem no SLA por serviço.
-- (Recria _dashboard_producao_core, que já traz a flag usaCorte da migration anterior.)
