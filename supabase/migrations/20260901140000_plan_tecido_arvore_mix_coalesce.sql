-- Fix: lane por mix não pegava os cards. modelos.mix_id é a FONTE (o Editar Mix/Mover grava
-- lá, update direto), mas o slot.mix_id só é setado no drag/vaga → dessincronia. A leitura da
-- árvore agora deriva o mix do slot como coalesce(sl.mix_id, m.mix_id): o slot HERDA o mix do
-- modelo vinculado. Conserta retroativo, sem sincronizar em toda escrita.
          ...
          select jsonb_agg(jsonb_build_object(
            ...
            'slots', coalesce((
              select jsonb_agg(jsonb_build_object(
                ...
                'categoria_id', sl.categoria_id, 'categoria_tecido_id', sl.categoria_tecido_id, 'mix_id', coalesce(sl.mix_id, m.mix_id),
