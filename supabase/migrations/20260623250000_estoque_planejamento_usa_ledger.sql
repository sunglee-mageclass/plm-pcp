-- estoque_tecido_por_artigo (físico/disponível do Planejamento) calculava a baixa
-- por metragem_enviada do CAD — que IGNORA as baixas de ajuste ('- Metragem') e de
-- separacao_rolo (gravam cad_id=NULL e não tocam metragem_enviada) e usa a metragem
-- cheia no lugar do que o corte realmente baixou (LEAST(restante,saldo) em déficit).
-- Resultado: físico do Planejamento divergia da fonte única (o ledger).
-- Correção: a CTE `baixa` passa a somar estoque_tecido_baixas por artigo (igual a
-- detalhe_estoque_variante). De quebra, o `recebido` ganha a mesma exclusão do
-- substituto-a-receber dos outros 3 RPCs (substitui_item_id + recebida NULL = 0),
-- pois usava o mesmo padrão COALESCE(recebida, pedida) -> estoque-fantasma.
