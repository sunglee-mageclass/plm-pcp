-- Plan. Tecido: ROLO vinculado credita COBERTURA (só o saldo: pedida − separacao_rolo), como uma OC.
-- O rolo entra no MESMO seletor de OC (front). Anti-dupla-contagem: o saldo do rolo SAI do estoque_m
-- da prévia (o rolo recebido já está no _estoque_tecido_core) e vira 'coberto', contando UMA vez.
-- Mudança LOCAL à prévia — NÃO toca no _estoque_tecido_core (SSOT de estoque, invariante #4).
