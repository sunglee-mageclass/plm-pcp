-- Produtos Importados — Fase 2: o `real` do custo passa a vir da OC recebida.
-- Atualiza a CTE `imp` + os ramos 'importado' de `_custo_unitario_modelos_core`:
--   - `imp` agora junta `ocs_importado` (LEFT JOIN por produto_importado_id) e traz o status da OC
--     + o custo landed REAL da OC (`custo_unitario_landed_real`, recalculado das cotações efetivas
--     das etapas da OC — que são editáveis conforme o câmbio real de cada pagamento).
--   - `real` = quando a OC está 'recebido' → landed real da OC; senão NULL (só previsto do card).
--   - `confirmado` = (OC recebida).
--   - `previsto` (landed do CARD) INTOCADO.
-- Caminho revenda e manufaturado byte-a-byte idênticos (diff-validado). _core segue com EXECUTE
-- revogado (#9). Idempotente + BEGIN/COMMIT.
