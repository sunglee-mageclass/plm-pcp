-- Etapas PL S5, Task 2: salvar_terceirizados passa a persistir peca_foto/peca_foto_data
-- (colunas adicionadas na Task 1) a partir do jsonb de cada bloco.
-- CREATE OR REPLACE sobre a definição VIVA da função (dump em /tmp/st5_before.sql) —
-- a ÚNICA mudança é a adição das 2 colunas ao UPDATE SET (após nf_entrada) e ao
-- INSERT (lista de colunas + VALUES), mesmo padrão do trio nf_saida/nf_entrada feito
-- na migration 20260824170000.
