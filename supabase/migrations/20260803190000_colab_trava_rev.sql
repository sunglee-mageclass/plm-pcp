-- 20260803190000_colab_trava_rev.sql
-- Concorrência multi-usuário (spec 2026-08-03), Task 2: trava otimista (_rev_base → P0409)
-- em salvar_oc_tecido / salvar_modelo_bom / salvar_plan_tecido.
--
-- Consome rev/plan_rev da Task 1 (20260803180000_colab_rev_infra.sql). Cada wrapper/_core
-- ganha um 4º/5º parâmetro `_rev_base int default null`: se informado e diferente do rev
-- atual do agregado-raiz (lido `FOR UPDATE`), a função dá RAISE com ERRCODE 'P0409' antes
-- de tocar no payload. `_rev_base` null preserva o comportamento antigo (sem trava) — as
-- telas que ainda chamam com 3/4 args caem no default.
--
-- CREATE OR REPLACE com assinatura diferente criaria OVERLOAD (não substitui a antiga) —
-- por isso DROP FUNCTION nas assinaturas antigas (wrapper e core) antes de recriar.
-- Envolvido em BEGIN/COMMIT porque há DROPs (tudo ou nada).

BEGIN;

-- ============================================================================
-- 1) salvar_oc_tecido / _salvar_oc_tecido_core
-- ============================================================================

DROP FUNCTION IF EXISTS public.salvar_oc_tecido(uuid, jsonb, jsonb);
DROP FUNCTION IF EXISTS public._salvar_oc_tecido_core(uuid, jsonb, jsonb);

CREATE FUNCTION public._salvar_oc_tecido_core(_oc_id uuid, _oc jsonb, _itens jsonb, _rev_base int default null)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  if _rev_base is not null then
    declare v_rev int;
    begin
      select rev into v_rev from public.ocs_tecido where id = _oc_id for update;
...
CREATE FUNCTION public.salvar_oc_tecido(_oc_id uuid, _oc jsonb, _itens jsonb, _rev_base int default null)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ============================================================================
-- 2) salvar_modelo_bom / _salvar_modelo_bom_core
-- ============================================================================

DROP FUNCTION IF EXISTS public.salvar_modelo_bom(uuid, jsonb, jsonb, jsonb);
DROP FUNCTION IF EXISTS public._salvar_modelo_bom_core(uuid, jsonb, jsonb, jsonb);

CREATE FUNCTION public._salvar_modelo_bom_core(_modelo_id uuid, _tecidos jsonb, _aviamentos jsonb, _grades jsonb, _rev_base int default null)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  if _rev_base is not null then
    declare v_rev int;
    begin
      select rev into v_rev from public.modelos where id = _modelo_id for update;
      if v_rev is distinct from _rev_base then
        raise exception 'conflito_versao: o registro foi salvo por outra pessoa'
          using errcode = 'P0409';
      end if;
...
CREATE FUNCTION public.salvar_modelo_bom(_modelo_id uuid, _tecidos jsonb, _aviamentos jsonb, _grades jsonb, _rev_base int default null)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ============================================================================
-- 3) salvar_plan_tecido / _salvar_plan_tecido_core
-- ============================================================================

DROP FUNCTION IF EXISTS public.salvar_plan_tecido(uuid, jsonb);
DROP FUNCTION IF EXISTS public._salvar_plan_tecido_core(uuid, jsonb);

CREATE FUNCTION public._salvar_plan_tecido_core(_colecao_id uuid, _arvore jsonb, _rev_base int default null)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
begin
  ...
  if _rev_base is not null then
    declare v_rev int;
    begin
      select plan_rev into v_rev from public.colecoes where id = _colecao_id for update;
      if v_rev is distinct from _rev_base then
        raise exception 'conflito_versao: o registro foi salvo por outra pessoa'
          using errcode = 'P0409';
      end if;
  ...
  -- bump da árvore do Plan. Tecido (spec 2026-08-03, decisão Task 1: escritor único,
  -- bump feito aqui na RPC em vez de trigger multi-nível)
  update public.colecoes set id = id where id = _colecao_id;

  return v_plan;
end $function$;

CREATE FUNCTION public.salvar_plan_tecido(_colecao_id uuid, _arvore jsonb, _rev_base int default null)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ============================================================================
-- 4) ACL (invariante #9) — cada _core novo revogado de PUBLIC/anon/authenticated;
--    cada wrapper novo concedido a authenticated.
-- ============================================================================

REVOKE EXECUTE ON FUNCTION public._salvar_oc_tecido_core(uuid, jsonb, jsonb, int) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public._salvar_modelo_bom_core(uuid, jsonb, jsonb, jsonb, int) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public._salvar_plan_tecido_core(uuid, jsonb, int) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.salvar_oc_tecido(uuid, jsonb, jsonb, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.salvar_modelo_bom(uuid, jsonb, jsonb, jsonb, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.salvar_plan_tecido(uuid, jsonb, int) TO authenticated;
