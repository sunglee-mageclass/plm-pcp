-- Prévia do pedido: expõe a COBERTURA por variante (todas as linhas da conta, inclusive
-- déficit 0) — o drawer "A comprar" ganha a equação Nec − Coberto = A comprar nas colunas
-- (mesmo padrão aprovado do drawer Situação); antes só saíam os itens com déficit > 0 e o
-- usuário subtraía de cabeça (laudo das 3 lentes, jul/2026). Chave ADITIVA no jsonb.
