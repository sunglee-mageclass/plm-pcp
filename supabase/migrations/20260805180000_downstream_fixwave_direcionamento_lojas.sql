-- Direcionamento multi-lojas — FIX WAVE final (achados do review de branch pós-Task 7):
-- 3 pontos que já cuidavam do downstream do Direcionamento tocavam SÓ a tabela LEGADA
-- `direcionamento`, deixando `direcionamento_lojas` (o modelo novo, desde a Task 3) fora
-- do alcance. Mesma classe de fix da migração 20260805170000 (modelo_etapas_afetadas /
-- marcar_revisao_por_mudanca). CREATE OR REPLACE nas 4 funções, corpo extraído AO VIVO via
-- pg_get_functiondef e diff-validado antes/depois (só a linha indicada muda em cada uma):
--
-- 1. `_reverter_corte_tecido_core` — o DELETE defensivo em `direcionamento` ganha o
--    irmão em `direcionamento_lojas` (reverter o corte de um CAD direcionado só no
--    modelo novo deixava a linha órfã lá).
-- 2. `_voltar_cq_para_servico_core` — mesmo padrão (delete defensivo nas duas tabelas).
-- 3. `_poda_variantes_cad` — a poda por variante removida do Tecido 1 já limpava
--    `cad_grades`/`modelo_grades`/`direcionamento`/`cq_variantes`/`cq_pos_variantes`, mas
--    deixava a linha da variante removida órfã em `direcionamento_lojas` — o PRODUTOR real
--    das linhas órfãs que bloqueavam "Confirmar" no front e `excluir_loja_direcionamento`.
-- 4. `_dashboard_producao_core` — a timeline só acendia a etapa "Direcionamento" via
--    EXISTS(direcionamento); ganha OR EXISTS(direcionamento_lojas), mesmo alias/filtro
--    (`dl.cad_id = cd.id`) usado no fix da 20260805170000.
--
-- ACL: as 4 já tinham EXECUTE revogado de PUBLIC/anon/authenticated (confirmado via
-- has_function_privilege antes da migração). CREATE OR REPLACE preserva ACL de função
-- existente (mesma assinatura) — mas reasserta-se o REVOKE aqui mesmo assim, seguindo o
-- padrão do repo (ex.: 20260805160000) de nunca depender silenciosamente disso.
BEGIN;
