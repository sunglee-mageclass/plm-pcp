-- Fix (set/2026): o save em LOTE `salvar_produto_acabado` grava markup sem zerar o preço fixo do
-- canal — quebra "última edição manda" e reabre o "298→297,84" no próximo ciclo (achado da revisão).
-- Recria `_salvar_produto_acabado_core` = def vigente + limpa `preco_*_fixo` do canal quando o markup
-- vem não-null (só o UPDATE; INSERT cria produto novo, sem fixo). Diff-validado antes/depois.
BEGIN;
