-- Fase 3 — Onda OTB (parte 1): merge de conflito nos editores de Coleção (orçamento) e Poder de Venda.
--
-- Âncora = `colecoes` (já publicada no Realtime desde 20260803180000). ⚠️ NÃO reusar `colecoes.plan_rev`
-- (é do fluxo Plan. Tecido, bumpado por outros triggers — misturaria semânticas e daria falso-positivo
-- de "mudou" no editor OTB quando alguém salva Plan. Tecido). Coluna DEDICADA `otb_rev` + trigger próprio.
--
-- O `_rev_base` viaja DENTRO do jsonb (`_payload->>'rev_base'` / `_header->>'rev_base'`) — assim NÃO
-- muda a assinatura das RPCs (evita overload). null/ausente = bypass (criação, ou save legado).
--
-- Ambas as RPCs fazem `UPDATE colecoes` no ramo de edição → o trigger BEFORE UPDATE bumpa o otb_rev
-- (mesmo quando só as filhas mudam, o UPDATE do cabeçalho sempre roda). Diff-validar byte-a-byte: só
-- ADICIONA o bloco de trava (P0409) no ramo de edição; toda a lógica de subcoleções/semanas/itens/
-- guarda-de-Plan.Tecido INTACTA.

BEGIN;

-- ── otb_rev dedicado + trigger de bump ────────────────────────────────────────
ALTER TABLE public.colecoes ADD COLUMN IF NOT EXISTS otb_rev integer NOT NULL DEFAULT 0;

CREATE OR REPLACE FUNCTION public.fn_colab_touch_otb_rev()
 RETURNS trigger LANGUAGE plpgsql
AS $function$
BEGIN
  NEW.otb_rev := COALESCE(OLD.otb_rev, 0) + 1;
  RETURN NEW;
END $function$;

DROP TRIGGER IF EXISTS trg_colab_otb_rev ON public.colecoes;
CREATE TRIGGER trg_colab_otb_rev
  BEFORE UPDATE ON public.colecoes
  FOR EACH ROW EXECUTE FUNCTION public.fn_colab_touch_otb_rev();

-- ── otb_salvar_colecao + trava P0409 (rev_base no payload) ────────────────────
-- ⚠️ Corpo COPIADO BYTE-A-BYTE do pg_get_functiondef vigente (diff-validado): só ADICIONA
-- v_rev_base/v_rev nas declares + o bloco de trava no ramo de edição. NADA MAIS muda.
...
-- ── salvar_colecao_pv + trava P0409 (rev_base no header) ──────────────────────
...
declare
  ...
  v_claimed uuid[] := '{}';
  v_orfa text;
  v_rev_base int := nullif(_header->>'rev_base','')::int;
  v_rev int;
begin
  ...
  if v_id is null then
    ...
    -- trava otimista (Fase 3) — P0409 se outra pessoa salvou no meio. rev_base null = bypass.
    if v_rev_base is not null then
      select otb_rev into v_rev from public.colecoes where id = v_id and tipo = 'poder_venda' for update;
      if v_rev is distinct from v_rev_base then
        raise exception 'conflito_versao: o registro foi salvo por outra pessoa' using errcode='P0409';
      end if;
    end if;
