-- FORNECEDORES — FASE 5a (segura/reversível): migra as RPCs de LEITURA de terceirizado_id
-- para empresa_id (o espelho garante nome idêntico → saída equivalente). Assim a F5b pode
-- dropar terceirizados sem quebrar ranking/dashboard/CQ. NADA é dropado aqui.
...
BEGIN
  ...
  WITH entregas AS (
    SELECT ter.nome_fantasia AS oficina,
    ...
    JOIN empresas ter ON ter.id = t.empresa_id
...
BEGIN
  ...
  SELECT to_jsonb(t) INTO v_out FROM (
    SELECT pt.id,
           COALESCE(ter.nome_fantasia, col.nome, '—') AS responsavel,
    ...
    LEFT JOIN empresas ter ON ter.id = pt.empresa_id
