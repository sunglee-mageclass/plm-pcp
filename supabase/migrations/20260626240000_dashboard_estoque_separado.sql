-- Dashboard > Estoque: em vez de "Estoque Crítico" (10 menores, combinado tecido+aviamento)
-- + 1 grafico so de tecido, a RPC passa a devolver:
--   - estoqueTecido / estoqueAviamento: listas SEPARADAS, do MAIOR para o MENOR (top 50);
--   - barTecido / barAviamento: estoque por categoria (aviamento agora usa categorias_aviamento
--     REAIS, nao mais o rotulo flat 'Aviamento').
-- Sai o conceito de "critico/threshold" (a loja nao tem alerta de minimo).
...
BEGIN
  ...
  avi AS (
    SELECT a.id, a.codigo_nome AS nome,
           COALESCE(cav.nome,'Sem categoria') AS categoria,
