-- Leadtime: Serviços macro OU micro. Além do macro "Produção (Serviços)" (corte → última
-- entrega), permite acompanhar CADA categoria de serviço como etapa própria (micro):
-- key 'servico_cat:<id>', tipo 'servico', duração = data_entregue − data_enviado do bloco.
-- Micro só aparece quando explicitamente configurado (bootstrap sem config = só o macro).
-- Carrega 'sub' = categorias_terceirizado.ordem p/ o front ordenar as categorias no fluxo.
