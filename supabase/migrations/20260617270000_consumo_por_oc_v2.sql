-- Consumo por OC v2: enriquece cada modelo com os números do mini-card
-- (consumo unitário, multiplicador, grade por variante e grade total geral),
-- além do consumo_m (= consumo × mult × (grade_variante + 1 piloto), sem perda).
              ...
              'modelos', (
                SELECT COALESCE(jsonb_agg(m_row), '[]'::jsonb) FROM (
                  ...
                  SELECT jsonb_build_object(
                    ...
                    'mult', COALESCE((SELECT mtv.multiplicador FROM public.modelo_tecido_variantes mtv WHERE mtv.modelo_tecido_id = mt.id AND mtv.ordem = l.ordem),1),
                    'grade_variante', COALESCE((SELECT mg.grade_total FROM public.modelo_grades mg WHERE mg.modelo_id = l.modelo_id AND mg.variante_numero = l.ordem),0),
                    'grade_geral', COALESCE((SELECT SUM(mg.grade_total) FROM public.modelo_grades mg WHERE mg.modelo_id = l.modelo_id),0),
                  ...
                  SELECT jsonb_build_object(
                    ...
                    'mult', NULL,
                    'grade_variante', NULL,
                    'grade_geral', COALESCE((SELECT SUM(mg.grade_total) FROM public.modelo_grades mg WHERE mg.modelo_id = md.id),0),
