-- Grade Cortada — FONTE ÚNICA, fix1 (review adversarial da Task 3).
--
-- F1 (MÉDIO): o guard [C1] ("não confirma com Σ Grade Real = 0") computava v_total_real do _reais
--   do CLIENTE, mas com bloco-fonte a Grade Real autoritativa deriva do grade_detalhe
--   (Σ max(0, recebida−defeito)). Isso furava o [C1] (fonte real=0 + _reais mentindo confirmava) e
--   bloqueava indevidamente (fonte real>0 + _reais vazio). Fix: com fonte, o [C1] usa o MESMO
--   grade_detalhe da fonte. Para isso a mescla do payload no grade_detalhe (v_gd) é feita ANTES do
--   [C1]; se o [C1] abortar, o UPDATE é revertido junto (mesma txn atômica).
--
-- F2 (BAIXO): o rollup de qtd no bloco-fonte dispara auto_status_terceirizado, que rebaixa o status
--   p/ 'pendente' quando o escalar quantidade_enviada é nulo (branch 'finalizado' exige
--   quantidade_enviada>0). Fix: no MESMO rollup recompute quantidade_enviada = Σ grade_detalhe.enviada
--   (bloco-fonte é sempre detalhado ⇒ a grade é a fonte), mantendo o auto_status coerente.
--
-- Só redefine _salvar_cq_core; helpers e salvar_terceirizados inalterados. Idempotente (CREATE OR
-- REPLACE); ACL preservada (o _core segue com EXECUTE revogado de PUBLIC/anon/authenticated).
