-- Marca revisão pendente (#Erro) calculando as etapas atingidas no servidor
-- (mais robusto que depender do reached/flags no frontend). Grade/consumo/aviamentos
-- editados → marca as etapas de produção que existem para o modelo.
CREATE OR REPLACE FUNCTION public.marcar_revisao_por_mudanca(
  _modelo_id uuid, _grade boolean, _consumo boolean, _aviamentos boolean
) RETURNS jsonb
...
BEGIN
  ...
  -- etapas de produção que o modelo já alcançou (grade/consumo/aviamentos podem
  -- deixar as quantidades/metragens dessas etapas inconsistentes).
...
REVOKE EXECUTE ON FUNCTION public.marcar_revisao_por_mudanca(uuid, boolean, boolean, boolean) FROM anon;
