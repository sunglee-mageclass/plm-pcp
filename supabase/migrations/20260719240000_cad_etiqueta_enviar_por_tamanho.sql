-- Etiquetas Fase 1d.2: Qtd a Enviar POR VARIANTE (por tamanho) no CAD.
-- cad_etiquetas.enviar_por_tamanho jsonb {tamanho: qtd}. Recria _salvar_cad_completo_core
-- (def viva) so com o campo novo no INSERT das etiquetas.

BEGIN;

ALTER TABLE public.cad_etiquetas ADD COLUMN IF NOT EXISTS enviar_por_tamanho jsonb NOT NULL DEFAULT '{}'::jsonb;
