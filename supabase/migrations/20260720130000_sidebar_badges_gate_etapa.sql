-- Item 5a/5b: o badge de #Erro contava por revisao_pendente sem checar se o modelo está
-- REALMENTE na etapa → badge mostrava número com a tela vazia (ex.: modelo com serviços mas
-- não cortado, ou revertido). Alinha as 3 contagens aos gates reais das listas:
--   Serviços e CQ: só entram após o corte (cad.enviado_corte=true).
--   Direcionamento: só entra com CQ liberado (_cq_liberado).
BEGIN;
...
DECLARE
  ...
  v_err_terc int; v_err_cq int; v_err_dir int;
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'erro_direcionamento', COALESCE(v_err_dir,0));
END $function$;
