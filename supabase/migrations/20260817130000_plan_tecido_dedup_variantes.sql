-- Plan. Tecido — DEDUP de variante repetida por material no SERVIDOR (camada autoritativa).
--
-- Bug (Loja Teste / Resort 27): o slot e69b9afe / material 0b544a5a tinha a variante
-- bf3d513f DUPLICADA em plan_tecido_variantes (mesma cor real 2× no mesmo material, ordens
-- 2 e 3, grade_total 60/60). Como _plan_tecido_nec_variante_core soma consumo × grade_total
-- por (artigo, variante) sobre TODAS as linhas, a necessidade dessa cor contava 2× (14,4 m
-- em vez de 7,2 m). Os painéis do front (slotMetros / necessidadePorTecido / detalheOc)
-- também dobravam. O fix de key do React (da0ed99) só calou o warning "two children with the
-- same key" — o DADO seguia duplicado e contando 2×.
--
-- Causa: _salvar_plan_tecido_core inseria CADA elemento de v_mat->'variantes' sem deduplicar
-- (aceitava 2 linhas da mesma variante no mesmo material). A UNIQUE só cobre (material_id, ordem),
-- nunca (material_id, variante). O front tem uma rede (comVariantesDoPlano dedup por
-- variante_tecido_id), mas ela só age no caminho "BOM vivo sem variantes" e o auto-upgrade de cor
-- planejada→real (MaterialBlock) podia remapear 2 linhas distintas pra MESMA variante sem colapsar.
--
-- Fix (defesa em profundidade): ao inserir as variantes de um material, colapsar por identidade:
--   • cor REAL   → chave = variante_tecido_id
--   • cor PLANEJADA (sem variante) com cor/apelido → chave = (cor_id, cor_apelido_id)
--   • linha SEM identidade (variante e cor ambos nulos) → NUNCA colapsa (chave por posição)
-- Regra MENOS DESTRUTIVA: mantém a linha de MAIOR grade_total (empate → menor ordem/posição
-- original), preservando os `grades` daquela linha. NUNCA soma grade_total — somar recriaria
-- exatamente a inflação 2× que estamos consertando (a duplicata é uma anomalia "mesma cor 2×",
-- não duas demandas distintas; uma cor num material tem UMA grade autoritativa). Ordem renumerada
-- 1..n compacta (respeita uq_plan_var(material_id, ordem)).
--
-- Espelha o helper puro dedupVariantes (src/lib/plan-tecido/calc.ts) usado pelo card do front.
-- Resto da função inalterado (diff-validar pg_get_functiondef antes/depois).
...
-- invariante #9: _core nunca executável direto (só via wrapper salvar_plan_tecido)
