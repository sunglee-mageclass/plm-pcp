-- 1) Tabela modelo_tecido_oc_links: múltiplas OCs por variante
...
-- tenta variações de nome geradas pelo Postgres
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    ...
    WHERE conrelid = 'public.modelo_tecido_oc_links'::regclass
      AND contype = 'u'
  LOOP
    -- mantém apenas a nova unique; remove qualquer outra
    IF r.conname NOT IN ('modelo_tecido_oc_links_unique_oc') THEN
      EXECUTE format('ALTER TABLE public.modelo_tecido_oc_links DROP CONSTRAINT %I', r.conname);
    END IF;
  END LOOP;
END $$;
...
ALTER TABLE public.modelo_tecido_oc_links
  ADD CONSTRAINT modelo_tecido_oc_links_unique_oc
  UNIQUE (modelo_id, tipo, numero, ordem, oc_tecido_item_id);

-- 2) ocs_disponiveis_variante: todas OCs (recebidas+previstas), sem filtrar saldo
...
-- 3) salvar_modelo_bom: grava quantidade_m e prioridade
...
-- 4) baixar_estoque_tecido_corte: percorre vínculos por prioridade
