...
```
⚠️ **A confirmar:** a lista de PCP do dono tem 4 itens. Hoje "Produção" também tem **Oficina** e a página de **Alertas de tecido** (dentro de Consumo por OC/OC Tecido). Decidir: Oficina fica em PCP (não listada só por brevidade), some, ou mescla em Serviços? Não dropar tela sem confirmar.
