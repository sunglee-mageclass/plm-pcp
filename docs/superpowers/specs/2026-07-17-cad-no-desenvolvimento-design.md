...
## 11. Revisão crítica — refinamentos OBRIGATÓRIOS (red-team de 3 lentes)
A revisão pegou furos reais. Resolvidos assim (a maioria **simplifica**):

1. **A Explosão/corte NÃO escreve `grades_reais`.** A grade real já é **semeada = grade cheia na criação do `cad`** (no "Enviar" — `enviar_modelo_para_cad` copia `planejada=real`). Então "a grade vira real" acontece **no Enviar** (não precisa o corte escrever de novo). O corte só faz a baixa; o CQ **refina** por defeito (como hoje). → **Elimina a dupla-escrita `grades_reais` (corte × CQ)**, que seria o furo mais grave.
2. **Card TRAVA o BOM/grade após "Enviar"** (é o comportamento atual: `locked = enviado_cad && !editing`). Assim o snapshot `cad_*` não fica **stale** (não há edição de `modelo_*` depois do Enviar). Para mexer, o usuário **reverte o Enviar** (destrava). → **Elimina o furo de staleness `cad_*` × `modelo_*`** sem re-sync bidirecional.
3. **Cálculo do "4. CAD" (folhas/tamanho da folha) precisa de persistência ANTES do `cad`.** Hoje esses campos vivem em `cad_tecido_variantes` (só existem com `cad`) e o cálculo é local no front. **Refinamento:** guardar o rascunho em **colunas novas em `modelo_tecido_variantes`** (`tamanho_folha`, `qtd_folhas` — aditivo) editáveis no card; o "Enviar" copia pro `cad_*`. → resolve "abre a aba, some o número".
4. **Fichas imprimem do `modelo_*` OU só após "Enviar".** Evitar o fallback complexo do `useFichaData` (shapes de `cad_aviamentos` ≠ `modelo_aviamentos`). Decisão do plano: (a) `useFichaData` com fallback real p/ `modelo_*`, ou (b) botão de imprimir habilita **após Enviar** (quando `cad_*` existe). Recomendo (b) — mais simples e o CAD calc já pressupõe a etapa.
5. **Custo congelado (`cad_tecidos.custo_cad`)**: garantir que o "Enviar" (snapshot) congela o custo via preço da OC vinculada (Fase B), e que re-salvar não zera. Cobrir com teste.
6. **Sidebar — gate × grupo (Consumo por OC):** `producao_consumo_oc` está amarrado ao módulo `producao`. Para renderizar no grupo Estilo & Engenharia **sem** sumir quando `producao` estiver ligado mas exibido em `criacao`: criar entrada **`criacao_consumo_oc`** em `PAGES_CATALOG` (grupo criacao) **reusando a permissão/rota** existente (mínima duplicação), OU suportar `crossModules`. Gate de módulo a decidir (fica em `producao` ou passa a `criacao`?).
7. **Rename × `tabLabels`:** o rename em `MODULE_META` é o **default**; se a loja-alvo tem `tenant_config.tab_labels` fixando "Criação"/"Produção", o override vence — **limpar as chaves** `criacao`/`producao` do tab_labels da loja (ou avisar admin).
