...
- **Notas do review da F1 (resolver na F2):**
  - **⚠️ CORRIGIDO (review architect+domain): NÃO trocar p/ escrita-empresa; escrita é DUAL via GATILHO.** A Produção lê `terceirizado_id` até a F3 (e `_ranking_oficinas_core` usa INNER JOIN → serviço sem terceirizado SOME do ranking/SLA sem erro). Então a F2 mantém o espelho `terceirizados`/`terceirizado_categorias` sincronizado por **gatilho forward-only** empresa(servico)→terceirizado (mirror `20260707220000`), com loop-guard GUC `app.terc_mirror`. A tela só mexe em `empresas`; o gatilho espelha (incl. DELETE de categoria, sem fantasma). `origem_terceirizado_id` passa a ser "o id-espelho legado desta empresa serviço" (não só migradas F1). `producao_terceirizados.empresa_id` derivado por gatilho. Nada de RPC de leitura muda na F2 (isso é F3).
  - **CNPJ NÃO obrigatório em empresa de serviço** (interno/PL e pessoa física podem não ter). Situação cadastral/lookup só quando há CNPJ.
