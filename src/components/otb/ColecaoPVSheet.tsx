                              ...
                              {s.linhas.map((l) => {
                                ...
                                return (
                                  <tr key={l.id} className="border-t border-border/50 [&>td]:px-2 [&>td]:py-1 [&>td]:text-left">
                                    ...
                                    <td data-label="Preço mín"><MoneyInput className="h-8 w-20 max-md:h-11 px-1 text-left tabular-nums" value={l.min || ""} placeholder="0,00" onChange={(e) => patchLinha(s.id, l.id, { min: Number(e.target.value) || 0 })} /></td>
                                    <td data-label="Preço máx"><MoneyInput className="h-8 w-20 max-md:h-11 px-1 text-left tabular-nums" value={l.max || ""} placeholder="0,00" onChange={(e) => patchLinha(s.id, l.id, { max: Number(e.target.value) || 0 })} /></td>
                                    {s.semanas.map((w) => (
