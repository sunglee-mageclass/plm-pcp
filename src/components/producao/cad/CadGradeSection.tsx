...
export function CadGradeSection({ grades, tamanhosAll, updateGradeCell, labelByNumero, gradeAuto, onToggleGradeAuto }: Props) {
  return (
    <Card className="p-5 space-y-3">
      ...
      {gradeAuto && (
        <p className="text-[11px] text-muted-foreground">
          Digite um tamanho na linha "CAD" e os demais preenchem na proporção do modelo.
        </p>
      )}
      {grades.length === 0 && (
      ...
      {grades.map((g, gi) => (
        <div key={g.variante_numero} className="space-y-2">
          <div className="text-sm font-medium">{labelByNumero?.[g.variante_numero] ?? `Variante ${g.variante_numero}`}</div>
                ...
                <tr className="border-t">
                  ...
                  {tamanhosAll.map((t) => (
                    <td key={t} className="px-2 py-1">
                      <Input type="number" readOnly tabIndex={-1} className="bg-muted"
                        value={g.grades_planejadas[t] ?? 0} />
                    </td>
                  ))}
                  <td className="px-2 py-1 font-medium text-center">{g.grade_total_planejada}</td>
                </tr>
                <tr className="border-t">
                  <td className="px-2 py-1 text-muted-foreground">CAD</td>
                  {tamanhosAll.map((t) => (
                    <td key={t} className="px-2 py-1">
