...
/**
 * Dialog de destino do "Replicar card(s)" do Produto Importado — espelha
 * `ReplicarCardsDialog` (Plan. Tecido) quase igual. Dropdowns PESQUISÁVEIS (Popover+Input):
 * Coleção → Subcoleção. Ao confirmar, chama `onConfirmar(colecaoId, subcolecaoId|null)`. A
 * RPC `replicar_produtos_importados` só replica produtos JÁ MATERIALIZADOS (`modelo_id`
 * não-nulo) — os sem card são silenciosamente ignorados por ela; aqui contamos e avisamos
 * (`nIgnorados`) ANTES de chamar. É um Dialog central (ação "novo/criar", regra UI).
 */
...
export function ReplicarImportadoDialog({
  ...
  const { data: colecoes = [] } = useQuery({
    queryKey: ["produto-importado-replicar-colecoes"],
    enabled: open,
    queryFn: async () =>
  ...
  const { data: subs = [] } = useQuery({
    queryKey: ["produto-importado-replicar-subs", colId],
    enabled: open && !!colId,
    queryFn: async () =>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-1">
          ...
          <p className="text-xs text-muted-foreground">
            {nEleg} card(s) → <b>{colNome}</b> › <b>{subNome}</b>. Copia variantes, etapas de
