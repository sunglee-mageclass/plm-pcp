...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring cursor-pointer transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 disabled:cursor-not-allowed group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[active=true]:shadow-[inset_3px_0_0_oklch(0.72_0.1_248)] data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        // Toque 44px no mobile (drawer off-canvas): min-h-11 SÓ abaixo do breakpoint md,
        // altura desktop (rail) intocada. Sem efeito no rail recolhido (group-data-[collapsible=icon]
        // só existe no wrapper DESKTOP — o drawer mobile é uma árvore Sheet separada, sem esse atributo).
        default: "h-8 text-sm max-md:min-h-11",
        sm: "h-7 text-xs",
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        // Toque 44px no mobile (mesma régua do SidebarMenuButton acima): max-md:min-h-11 sem tocar
        // a altura desktop (h-7).
        ...
        "data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[active=true]:shadow-[inset_2px_0_0_oklch(0.72_0.1_248)]",
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        "group-data-[collapsible=icon]:hidden",
        className,
