 ...
 * junto da aposentadoria do flag "usar estoque existente"). Três classes, tokens §Q (sem cor solta):
 *  - Rolo   → estoque físico por rolo (mantém o selo "Rolo" de sempre): primary-tint, CAIXA ALTA.
 *  - do plano → OC gerada pelo "Fazer pedido" DESTA coleção (existe linha em plan_tecido_ocs
 *               ligando OC→coleção): primary-tint (é "nossa", o plano a criou).
 *  - já existia → OC que JÁ existia (avulsa ou de outra coleção), vinculada só p/ acompanhamento:
 *               tom azul/informativo (sky), distinto do primary.
 * `isRolo` vence `owned` (um rolo nunca é "do plano" — rolo não sai do Fazer pedido). `owned`
...
export function OcOrigemBadge({
  ...
  return owned ? (
    <span
      ...
      title="OC gerada pelo Fazer pedido desta coleção"
    >
      do plano
    </span>
  ) : (
