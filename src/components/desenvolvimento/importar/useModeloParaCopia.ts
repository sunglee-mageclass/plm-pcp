  ...
  return useQuery<ModeloParaCopia>({
    ...
    queryFn: async () => {
      const [m, tec, avi, etq, gra, obs] = await Promise.all([
        ...
        supabase.from("modelo_aviamentos").select("aviamento_id, variante_aviamento_id, consumo, loss_percent, custo_previsto").eq("modelo_id", modeloId!).order("numero"),
      ...
      return {
        ...
        blocks,
        aviamentos: ((avi.data ?? []) as any[]).map((r) => ({ aviamento_id: r.aviamento_id, variante_aviamento_id: r.variante_aviamento_id ?? null, consumo: Number(r.consumo ?? 0), loss_percent: Number(r.loss_percent ?? 0), custo_previsto: Number(r.custo_previsto ?? 0) })),
