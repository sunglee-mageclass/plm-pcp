          ...
          <Card key={i} className="p-3 space-y-2">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              <FieldSelectOpt
                label="Etiqueta"
                value={r.etiqueta_id}
                onChange={(v) => onChangeRow(i, { etiqueta_id: v, cor_id: null })}
                options={etiquetas}
              />
              <FieldSelectOpt
                label={cores.length > 0 ? "Cor" : "Cor (sem variantes)"}
                value={r.cor_id}
                onChange={(v) => onChangeRow(i, { cor_id: v })}
                options={cores}
              />
              <Field label="Consumo">
                <NumberInput type="number" step="0.001" placeholder="0,000" value={r.consumo || ""} onChange={(e) => onChangeRow(i, { consumo: Number(e.target.value) || 0 })} />
              </Field>
              <Field label="% Loss">
                <NumberInput type="number" step="0.01" placeholder="0,00" value={r.loss_percent || ""} onChange={(e) => onChangeRow(i, { loss_percent: Number(e.target.value) || 0 })} />
              </Field>
