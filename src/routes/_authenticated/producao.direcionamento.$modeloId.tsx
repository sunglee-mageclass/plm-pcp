...
export function DirecionamentoDetail({ modeloId, onClose }: { modeloId: string; onClose?: () => void }) {
  ...
  const { data: cad } = useQuery({
    ...
    queryFn: async () => (await supabase.from("cad").select("id, direcionamento_status, direcionamento_confirmado_at" as never).eq("modelo_id", modeloId).maybeSingle()).data as { id: string; direcionamento_status: string | null; direcionamento_confirmado_at: string | null } | null,
  });
  useEffect(() => {
