...
import { ScrollText, ArrowLeft, ChevronRight, ChevronDown, RotateCcw } from "lucide-react";
      ...
      <header className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground">
            Registro de todos os eventos: o que foi criado, editado ou excluído, por quem e quando.
          </p>
        </div>
      </header>
      ...
      <Card>
        <CardContent className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={usuario} onChange={(e) => onFilter(() => setUsuario(e.target.value))} placeholder="Nome ou e-mail" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">De</Label>
            <Input type="date" value={dataIni} onChange={(e) => onFilter(() => setDataIni(e.target.value))} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Até</Label>
            <Input type="date" value={dataFim} onChange={(e) => onFilter(() => setDataFim(e.target.value))} />
          </div>
          {isSuperAdmin && (
            <div className="space-y-1.5">
